...
def parse_args():
    ...
    parser.add_argument("--eval_file", default="train/socialai_persona_v1_eval.jsonl")
    parser.add_argument("--reflex_file")
    parser.add_argument("--reflex_repeat", type=int, default=4)
    parser.add_argument("--output_dir", default="train/socialai_persona_14b_lora_v1")
...
def build_sft_sample(messages, tokenizer, max_seq_len):
    system_user_msgs = []
    assistant_msg = None
    ...
    available_assistant_len = max(max_seq_len - len(system_user_ids), 0)
...
def prepare_dataset(rows, tokenizer, max_seq_len):
    return Dataset.from_list(
        [build_sft_sample(row["messages"], tokenizer, max_seq_len) for row in rows]
    )
...
def main():
    ...
    use_gc = str(args.gradient_checkpointing).lower() == "true"
    ...
    if args.reflex_file:
        reflex_rows = read_jsonl(args.reflex_file)
        train_rows.extend(reflex_rows * max(args.reflex_repeat, 1))
    eval_rows = read_jsonl(args.eval_file)

    train_dataset = prepare_dataset(train_rows, tokenizer, args.max_seq_len)
    eval_dataset = prepare_dataset(eval_rows, tokenizer, args.max_seq_len)
