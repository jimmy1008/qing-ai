 ...
 * - 2 clear sentences in 45–160 chars → split into 2, only if both are substantial
...
bot.on("message", async (msg) => {
  ...
  if (hasSticker && !effectiveText) {
    ...
    inputText = `[貼圖:${emoji}] 對方用貼圖誇張地表達了某種情緒，不需要提及貼圖本身，直接回應那個情緒。`;
  } else if (hasGif && !effectiveText) {
    inputText = "[GIF] 對方傳了一個誇張的動態表情，直接回應那個誇張的反應，不需要提及GIF。";
  } else if (!effectiveText && (hasVideo || hasVoice)) {
    if (channel === "private") {
      await bot.sendMessage(chatId, "收到了，但我目前還沒辦法聽語音或看影片耶", { reply_to_message_id: msg.message_id });
    }
    return;
    ...
    if (imageDescription) {
      ...
      inputText = caption
        ? `[使用者傳了一張圖片，你無法分析圖片內容，只有說明文字：「${caption}」，根據說明文字回應即可，不可憑空描述圖片]`
        : `[使用者傳了一張圖片，但你無法看見或描述其內容，請誠實告知]`;
    }
  } else {
  ...
  if (isForwarded) {
    ...
    if (channel === "private") {
      ...
      const senderName = msg.forward_from?.first_name || msg.forward_sender_name || msg.forward_from_chat?.title || "某人";
      inputText = `[使用者轉傳了一則來自「${senderName}」的訊息給你看，內容是：「${inputText}」。這不是使用者直接問你的問題，你對訊息中提到的任何現實狀況（維修、約定、事件、進度等）一無所知，不可捏造回答。自然地回應使用者分享這則訊息這件事本身。]`;
    }
  }
